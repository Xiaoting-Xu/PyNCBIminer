...
"AY":0, "TY":0.5, "CY":0.5, "GY":0, "RY":0, "YA":0, "YT":0.5, "YC":0.5, "YG":0, "YR":0, "YY":0.5, "YK":0, "YM":0, "YS":0, "YW":0, "YB":0, "YD":0, "YH":0, "YV":0, "YN":0.25, "Y-":0, "KY":0, "MY":0, "SY":0, "WY":0, "BY":0, "DY":0, "HY":0, "VY":0, "NY":0.25, "-Y":0, 
    ...
    @staticmethod    
    def create_kmer_dictionary(k): ## already implemented in sequence_indexer.py
    ...
    def calculate_PI(self, record1, record2, record1_ends=None, record2_ends=None, percent=True):
        ...
        for i in range(len(record)):
            ...
            if sequence[position_i] != "-":
                ...
                if count == position_in_raw:
                    ...
                    if direction == -1:
                        position_in_alignment = len(record.seq)-position_in_alignment+1
                    break
        try:        
    ...
    def get_consensus_sequence(self, record_iter):
        ...
        if len(record_iter) == 1:
            return record_iter[0].seq
    ...
    def get_kmer_similarity_matrix(self, records, k):
        ...
        ----------
        Parameters
        - records - a record_iter of sequences
        - k - the length of kmer
        -------
        Returns
        - simi_matrix - the similarity matrix of given input records evaluated by kmer
        """
        ## STEP 1: index all records
        sequence_indexer = Sequence_indexer(k)
        indexed_records = []
        for record in records:
            idx = sequence_indexer.index_record(record)
            indexed_records.append(idx)
        
        
        
        ## STEP 2: calculate kmer similarity for all pairs
        dim = len(indexed_records)
        simi_matrix = np.ones((dim,dim))
        
        for i in range(dim-1):
            for j in range(i+1, dim):
                similarity = self.calculate_kmer_similarity(indexed_records[i], indexed_records[j])
                simi_matrix[i,j] = similarity
                simi_matrix[j,i] = similarity
                
        return simi_matrix
        
    
    @staticmethod
    def calculate_kmer_similarity(array1, array2):
        """ using kmer to calculate the similarity between two indexed record (array)
        ----------
        Parameters
        - array1 - an indexed array of one input sequence
        - array2 - an indexed array of one input sequence
        -------
        Returns
        - similarity - the kmer similarity between two input seqs
        """
        i = 0
        j = 0
        count_identical = 0
        ...
        similarity = count_identical / min(len(array1),len(array2))
