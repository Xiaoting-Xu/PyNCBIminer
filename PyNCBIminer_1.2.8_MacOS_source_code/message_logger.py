...
class MessageLogger:
    ...
    def popup_error(self, which="all"):
        """ NOT AVAILABLE NOW """
        
        """ show in a message box all error messages if which=="all", otherwise latest error message
        ----------
        Parameters
        - which - show all error messages if which=="all", otherwise latest error
            Default: "all"
        """
        if len(self.__error_messages) == 0:
            return
        
        # create message box and widget params
        msgBox = QMessageBox()
        msgBox.setWindowTitle("ERROR MSG")
        msgBox.setStandardButtons(QMessageBox.Ok)
        
        # set message info to show
        if which == "all":
            msg_text = "\n".join(self.__error_messages)
        else:
            msg_text = self.__error_messages[-1]
        msgBox.setText(msg_text)
        
        # set icon representing error
    ...
    def popup_warning(self, which="all"):
        """ NOT AVAILABLE NOW """
        
        """ show in a message box all warning messages if which=="all", otherwise latest warning message
        ----------
        Parameters
        - which - show all warning messages if which=="all", otherwise latest warning
            Default: "all"
        """
        if len(self.__warning_messages) == 0:
            return
        ...
        # set message info to show
        if which == "all":
            msg_text = "\n".join(self.__warning_messages)
        else:
            msg_text = self.__warning_messages[-1]
        msgBox.setText(msg_text)
    ...
    def popup_message(self, message):
        ...
        """ show given message in a message box
        ----------
        Parameters
